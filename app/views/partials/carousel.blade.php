...
@foreach($carousel as $image)
	<figure class="image img-one">
		<img src="{{ secure_asset($image->image_1) }}" alt="{{ $image->image_1_description }}" />
		<figcaption>{{ $image->image_1_description }}</figcaption>
	</figure>
	<figure class="image img-two">
		<img src="{{ secure_asset($image->image_2) }}" alt="{{ $image->image_2_description }}" />
		<figcaption>{{ $image->image_2_description }}</figcaption>
	</figure>
	<figure class="image img-three">
		<img src="{{ secure_asset($image->image_3) }}" alt="{{ $image->image_3_description }}" />
